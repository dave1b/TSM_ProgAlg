...
void summation() {
	...
	sw.Stop();
	check("Parallel explicit reduction:", sum0, sum8, ts, sw.GetElapsedTimeMilliseconds());

	sw.Restart();
	const int64_t sum9 = sumPar3(arr);
	sw.Stop();
	check("Parallel implicit reduction:", sum0, sum9, ts, sw.GetElapsedTimeMilliseconds());
}