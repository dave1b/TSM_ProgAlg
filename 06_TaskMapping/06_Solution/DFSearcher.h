...
private:
	...
	void stealWorkFrom(DFSearcher& s) {
		...
		auto lambda = [this, &s] {
			// TODO
			// exchange bestSolution
			m_bestSolution.store(std::min(m_bestSolution, s.m_bestSolution));
			s.m_bestSolution.store(m_bestSolution);

			// remove every nThreads-th node from s.openList and move it to this openList
			const size_t nNodes = m_openList.size() + s.m_openList.size();
			const size_t skipLen = m_searchers->size() - 1;
			auto it = s.m_openList.begin();

			while (it != s.m_openList.end()) {
				auto it2 = it;
				if (it2 != s.m_openList.end()) it2++;
				m_openList.splice(m_openList.end(), s.m_openList, it); // move *it from s.m_openList to the end of m_openList (without copying Nodes)
				it = it2;	// restores correct it value, because splice doesn't set it correctly

				// skip elements
				size_t i = skipLen;
				while (i >= 0 && it != s.m_openList.end()) it++;
			}
			assert(nNodes == m_openList.size() + s.m_openList.size());
			m_outOfWork = m_openList.empty();
		};
